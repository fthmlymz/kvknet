...
type
  TfrmKVKKDokumanTipSec = class(TUniForm)
    grdList: TUniDBGrid;
...
function frmKVKKDokumanTipSec: TfrmKVKKDokumanTipSec;
...
function frmKVKKDokumanTipSec: TfrmKVKKDokumanTipSec;
begin
  Result := TfrmKVKKDokumanTipSec(MainMod.GetFormInstance(TfrmKVKKDokumanTipSec));
end;

procedure TfrmKVKKDokumanTipSec.btnCancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfrmKVKKDokumanTipSec.btnSelectClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKDokumanTipSec.edSrcTextAjaxEvent(Sender: TComponent; EventName: string; Params: TUniStrings);
begin
  if EventName = 'doSearch' then NewSQL;
end;

procedure TfrmKVKKDokumanTipSec.grdListDblClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKDokumanTipSec.NewSQL;
var
  SQLStr : String;
  SrcText : string;
begin
  ...
  SQLStr := 'SELECT * FROM def_data_dokumantip WHERE mc_id =:mc_id AND ';
  ...
  begin
    ...
    SQLStr := SQLStr + 'dokumantip LIKE :SrcText AND ';
  end;

	System.Delete(SQLStr, Length(SQLStr) - 3, 4);
  SQLStr := SQLStr + ' ORDER BY dokumantip';
...
function TfrmKVKKDokumanTipSec.Select(AMultiSelect: Boolean = False): Integer;
begin
  Result := -1;
...
procedure TfrmKVKKDokumanTipSec.UniFormBeforeShow(Sender: TObject);
begin
  edSrcText.Text := '';
  NewSQL;

...
type
  TfrmKVKKSurecSec = class(TUniForm)
    grdList: TUniDBGrid;
...
function frmKVKKSurecSec: TfrmKVKKSurecSec;
...
function frmKVKKSurecSec: TfrmKVKKSurecSec;
begin
  Result := TfrmKVKKSurecSec(MainMod.GetFormInstance(TfrmKVKKSurecSec));
end;

procedure TfrmKVKKSurecSec.btnCancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfrmKVKKSurecSec.btnSelectClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKSurecSec.edSrcTextAjaxEvent(Sender: TComponent; EventName: string; Params: TUniStrings);
begin
  if EventName = 'doSearch' then NewSQL;
end;

procedure TfrmKVKKSurecSec.grdListDblClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKSurecSec.NewSQL;
var
  SQLStr : String;
  SrcText : string;
begin
  ...
  SQLStr := 'SELECT * FROM def_data_surec WHERE mc_id =:mc_id AND ';
  ...
  begin
    ...
    SQLStr := SQLStr + 'surec LIKE :SrcText AND ';
  end;

	System.Delete(SQLStr, Length(SQLStr) - 3, 4);
  SQLStr := SQLStr + ' ORDER BY surec';
...
function TfrmKVKKSurecSec.Select(AMultiSelect: Boolean = False): Integer;
begin
  Result := -1;
...
procedure TfrmKVKKSurecSec.UniFormBeforeShow(Sender: TObject);
begin
  edSrcText.Text := '';
  NewSQL;

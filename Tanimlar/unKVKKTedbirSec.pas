...
type
  TfrmKVKKTedbirSec = class(TUniForm)
    grdList: TUniDBGrid;
...
function frmKVKKTedbirSec: TfrmKVKKTedbirSec;
...
function frmKVKKTedbirSec: TfrmKVKKTedbirSec;
begin
  Result := TfrmKVKKTedbirSec(MainMod.GetFormInstance(TfrmKVKKTedbirSec));
end;

procedure TfrmKVKKTedbirSec.btnCancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfrmKVKKTedbirSec.btnSelectClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKTedbirSec.edSrcTextAjaxEvent(Sender: TComponent; EventName: string; Params: TUniStrings);
begin
  if EventName = 'doSearch' then NewSQL;
end;

procedure TfrmKVKKTedbirSec.grdListDblClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKTedbirSec.NewSQL;
var
  SQLStr : String;
  SrcText : string;
begin
  ...
  SQLStr := 'SELECT * FROM def_data_tedbir WHERE mc_id =:mc_id AND ';
  ...
  begin
    ...
    SQLStr := SQLStr + 'tedbir LIKE :SrcText AND ';
  end;

	System.Delete(SQLStr, Length(SQLStr) - 3, 4);
  SQLStr := SQLStr + ' ORDER BY tedbir';
...
function TfrmKVKKTedbirSec.Select(AMultiSelect: Boolean = False): Integer;
begin
  Result := -1;
...
procedure TfrmKVKKTedbirSec.UniFormBeforeShow(Sender: TObject);
begin
  edSrcText.Text := '';
  NewSQL;

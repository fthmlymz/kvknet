...
type
  TfrmKVKKDayanakSec = class(TUniForm)
    grdList: TUniDBGrid;
...
function frmKVKKDayanakSec: TfrmKVKKDayanakSec;
...
function frmKVKKDayanakSec: TfrmKVKKDayanakSec;
begin
  Result := TfrmKVKKDayanakSec(MainMod.GetFormInstance(TfrmKVKKDayanakSec));
end;

procedure TfrmKVKKDayanakSec.btnCancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfrmKVKKDayanakSec.btnSelectClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKDayanakSec.edSrcTextAjaxEvent(Sender: TComponent; EventName: string; Params: TUniStrings);
begin
  if EventName = 'doSearch' then NewSQL;
end;

procedure TfrmKVKKDayanakSec.grdListDblClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKDayanakSec.NewSQL;
var
  SQLStr : String;
  srctext :string;
begin
  ...
  SQLStr := 'SELECT * FROM def_data_dayanak WHERE mc_id =:mc_id AND ';
  ...
  begin
    ...
    SQLStr := SQLStr + 'dayanak LIKE :srctext AND ';
  end;

	System.Delete(SQLStr, Length(SQLStr) - 3, 4);
  SQLStr := SQLStr + ' ORDER BY dayanak';
//  QueryOpen(qDayanak, SQLStr);
...
function TfrmKVKKDayanakSec.Select(AMultiSelect: Boolean = False): Integer;
begin
  Result := -1;
...
procedure TfrmKVKKDayanakSec.UniFormBeforeShow(Sender: TObject);
begin
  edSrcText.Text := '';
  NewSQL;

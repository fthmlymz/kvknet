...
type
  TfrmKVKKKontrolSonucSec = class(TUniForm)
    grdList: TUniDBGrid;
...
function frmKVKKKontrolSonucSec: TfrmKVKKKontrolSonucSec;
...
function frmKVKKKontrolSonucSec: TfrmKVKKKontrolSonucSec;
begin
  Result := TfrmKVKKKontrolSonucSec(MainMod.GetFormInstance(TfrmKVKKKontrolSonucSec));
end;

procedure TfrmKVKKKontrolSonucSec.btnCancelClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TfrmKVKKKontrolSonucSec.btnSelectClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKKontrolSonucSec.edSrcTextAjaxEvent(Sender: TComponent; EventName: string; Params: TUniStrings);
begin
  if EventName = 'doSearch' then NewSQL;
end;

procedure TfrmKVKKKontrolSonucSec.grdListDblClick(Sender: TObject);
begin
  ModalResult := mrOk;
end;

procedure TfrmKVKKKontrolSonucSec.NewSQL;
var
  SQLStr : String;
  SrcText : string;
begin
  ...
  SQLStr := 'SELECT * FROM def_data_kontrolsonuc WHERE mc_id =:mc_id AND ';
  ...
  begin
    ...
    SQLStr := SQLStr + 'kontrolsonuc LIKE :SrcText AND ';
  end;

	System.Delete(SQLStr, Length(SQLStr) - 3, 4);
  SQLStr := SQLStr + ' ORDER BY kontrolsonuc';
...
function TfrmKVKKKontrolSonucSec.Select(AMultiSelect: Boolean = False): Integer;
begin
  Result := -1;
...
procedure TfrmKVKKKontrolSonucSec.UniFormBeforeShow(Sender: TObject);
begin
  edSrcText.Text := '';
  NewSQL;

...
begin
  ...
  begin
    ...
    tblKontrol.FieldByName('dynk_var').AsString    := 'H'; //dayanak
  ...
  QueryOpen(qTmp, 'SELECT DISTINCT ky.kaynak as kaynak_str, ks.kisi as kisi_str ' +
                  ' , case when cl.dayanak_ids<>'''' then ''E'' ELSE ''H'' END dynk ' +
                  'FROM data_checklist cl LEFT JOIN def_data_kaynak ky ON cl.kaynak_id=ky.id '+
                  ' LEFT JOIN def_data_kisi ks on cl.kisi_id=ks.id ' +
                  'WHERE  cl.active = ''E'' AND cl.mc_id = ' +
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if qTmp.FieldByName('dynk').AsString='E' then //üzerine yazacak
        begin
          tblKontrol.FieldByName('dynk_var').AsString := 'E';
          tblKontrol.FieldByName('mesaj_str').AsString  := 'Envanter ve Kontrol Formlarý Mevcut, Dayanak Mevcut';
        end else
        begin
          tblKontrol.FieldByName('dynk_var').AsString := 'H';
          tblKontrol.FieldByName('mesaj_str').AsString  := 'Envanter ve Kontrol Formlarý Mevcut, Dayanak Yok';
        end;
        tblKontrol.Post;
        SayHep := SayHep + 1;
        SayEnv := SayEnv - 1;
...
begin
  ...
  begin
    ...
    if  (qKontrol.FieldByName('durum').AsInteger in [2,3]) and
    (qKontrol.FieldByName('dynk_var').AsString<>'E') then
      Attribs.Font.Color := clRed; //$009797FF;
  end;
end;

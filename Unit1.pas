...
begin
  ...
  begin
    MessageDlg('Güvenlik Tedbiri belirtilmemiş. Lütfen kontrol ediniz.', mtError, [mbOk]);
    edTanim.SetFocus;
    Exit;
  end;
   // Doğan Akçay Takip Numarası (ID):	PYW-QUY-MS6X (Talep numarası: 579)
  ...
  begin
    MessageDlg('Daha Önce Aynı İsimde Güvenlik Tedbiri Tanımlanmıştır. Lütfen kontrol ediniz.', mtError, [mbOk]);
    edTanim.SetFocus;
    Exit;
  end;
...
begin
  ...
  begin
    if lbIDY.Text = '' then lbIDY.Text := 'SİSTEM';
    if lbSDY.Text = '' then lbSDY.Text := 'SİSTEM';
  end;

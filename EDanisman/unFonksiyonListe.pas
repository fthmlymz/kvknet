...
begin
  ...
  begin
	...
	WhereStr 	:= '(d.mc_id =:mc_id OR d.mc_id=0) AND ';
...
//	QueryOpen(qFonks, SQLStr);
  qFonks.SQL.Text := SQLStr ;
  qFonks.ParamByName('mc_id').AsString := inttostr(mainmod.MCID);
  if qFonks.FindParam('srctext') <> nil then
    qFonks.ParamByName('srctext').AsString := SrcText ;
 qFonks.Open;
  ManageSettings;

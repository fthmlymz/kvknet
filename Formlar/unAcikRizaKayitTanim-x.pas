...
uses uniGUIApplication, MainModule, McMLib, wsJETSMS, KvknetapiLib, unAcikRizaKayitListe,
  unAcikRizaDokumanKayit, unAcikRizaVKDurum, unHelpPage, ServerModule;
...
begin
  ...
  frmAcikRizaDokumanKayit.Exec(qARDoc, 0, qRiza.FieldByName('id').AsInteger, 0);
end;
...
begin
  ...
  qRiza.FieldByName('onay_sira').AsInteger  := radioOnaySira.ItemIndex;
...
//  qRiza.Post;
  ...
  if not SendSMS(Mesaj, GSMNo, True) then
  begin
    MessageDlg(Mesaj, mtError, [mbOk]);
    MainMod.SaveSMSLog(MainMod.MCID, qRiza.FieldByName('id').AsInteger, 0, 'ESAPL', '', 'HATA : ' + Mesaj);
    Exit;
  end;
...
begin
  ...
  begin
    ...
    qRiza.FieldByName('id_tur').AsString     := '1';
...
begin
  ...
  btnVKDelete.Enabled  := (qRiza.State = dsBrowse);
...
begin
  frmHelpPage.Exec('AcikRizaKayitTanim');
end;
...
begin
  ...
  begin
    Msg := 'Bu onay/rýza için veri kaynaklarý oluþturulmamýþ. Þimdi otomatik oluþturmak ister misiniz?<br><br>' +
           'NOT : Eðer manuel alýnmýþ rýza kaydý giriyorsanýz EVET seçiniz. Aksi durumda ONLINE açýk rýza ' +
